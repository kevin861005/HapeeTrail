-- T11-05：私人便條（audience）。旅人可以留下只給自己的便條，當作這趟旅程的紀錄。
--
-- audience 與 color/style 性質完全不同：後者是純呈現、後端不理解其語意；
-- audience 是**後端必須理解並據以過濾**的業務語意，因此不合法的值一律大聲失敗
-- ——靜默走預設會把旅人以為私密的便條變成公開的，且沒有任何訊號。
--
-- 用 text ＋ CHECK 而非資料庫的 enum 型別：enum 加值有交易限制、刪值幾乎不可能，
-- 而這個欄位的前提就是日後會加值。
alter table public.notes
  add column audience text not null default 'anyone',
  add constraint notes_audience_values check (audience in ('anyone', 'self'));

comment on column public.notes.audience is
  '誰撿得到：anyone ＝ 給任何人撿；self ＝ 只給自己的旅遊紀錄（不進探索、撿不走、不佔上限）。';

-- 探索的熱查詢索引加上 audience：私人便條永遠不會出現在探索結果中，排除在索引外
-- 可讓熱查詢的索引更小；且索引條件必須與查詢的 where 一致才會被查詢計畫採用
drop index public.notes_active_location_gix;
create index notes_active_location_gix on public.notes using gist (location)
  where picked_up_at is null and audience = 'anyone';

-- ─── 便條的形狀：多一個鍵（探索提示刻意不加——那裡永遠只有 anyone，是個常數）────
...
begin
  ...
  -- 探索會靜默退化成全表掃描（EXPLAIN 證據見 ticket 05）
  return (
...
-- ─── drop_note（新增一個可省略參數 ⇒ 簽名改變，先 drop）──────────────────────
drop function public.drop_note(text, double precision, double precision, integer, integer);
...
revoke execute on function
  public.drop_note(text, double precision, double precision, integer, integer, text)
  from public, anon;
