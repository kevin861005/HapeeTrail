...
| 5 | 10 首次部署 Fly | 開 Fly.io 帳號、`fly auth login`；準備 hosted 專案的 DB 密碼（設 `hapeetrail_api` 密碼要用） | `/implement .scratch/java-rewrite/issues/10-first-deploy-fly.md` | 真 GoTrue token 打 Fly 的 `/v1/me/notes` 200；pooler IPv4 連得上 |
...
| 11 | 11 驗收 | Fly 已部署最新版（session 會重新部署） | `/implement .scratch/java-rewrite/issues/11-acceptance-newman-smoke.md` | newman 對 Fly 30 輪 0 失敗；smoke 全過；**你通知夥伴環境可用** |
...
- 步 4：JWT 是 HS256 還是非對稱金鑰（共享密鑰永不進 Spring）。
- 步 5：Fly → Supabase session pooler 的 IPv4 可達性。
- 步 3：Supabase Postgres 映像自帶哪些（`auth.users` 缺就補最小 shim）。
